...
function archivarCasosCerrados() {
  ...
  // Obtener o crear hoja histórica
  ...
  HOJAS_DESTINO.forEach(nombreHoja => {
    ...
    // Si el histórico está vacío, copiamos los encabezados + columna extra "Origen" y "Fecha Archivado"
    ...
    // Recorremos de abajo hacia arriba para eliminar sin desfasar índices
    for (let i = 1; i < datos.length; i++) {
      const fila = datos[i];
      const estado = (fila[18] || "").toString().trim().toUpperCase();
      const notificar = (fila[20] || "").toString();

      // Solo se archivan los APROBADO o RECHAZADO
      if (estado !== "APROBADO" && estado !== "RECHAZADO") continue;

      // Necesitamos saber CUÁNDO se cerró. Lo extraemos de la columna NOTIFICAR si tiene fecha.
      // Si nunca se notificó, usamos el timestamp del formulario (columna A) como referencia.
      ...
      if (fechaReferencia && fechaReferencia < limite) {
        ...
        filasAEliminar.push(i + 1); // +1 porque Sheets es 1-indexed
      }
    }
    ...
    // Eliminar de la hoja operativa (de abajo hacia arriba)
...
/**
 * Función para correr manualmente desde el menú o un botón
 */
